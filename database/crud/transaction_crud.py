...
def create_transaction(db: Session, user_id: int, category_id: int, amount: float, type: str,
...
def get_transaction_by_type(db: Session, user_id: int, type: str) -> Transaction:
    return db.query(Transaction).filter(Transaction.type == type, Transaction.user_id == user_id).order_by(
...
def get_transaction_by_date(db: Session, user_id: int, date: datetime) -> Transaction:
    return db.query(Transaction).filter(Transaction.user_id == user_id, Transaction.created_on == date).order_by(
        Transaction.created_on.desc()).all()


def update_transaction(db: Session, transaction_id: int, user_id: int, amount: float = None, type: str = None,
                       description: str = None) -> Transaction:
...
def get_total_transaction_by_type(db: Session, user_id: int, transaction_type: str, startdate: datetime,
...
def get_transactions_by_category(db: Session, user_id: int, transaction_type: str, start_date: datetime,
