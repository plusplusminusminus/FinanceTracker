...
from app.transactions import TransactionService
...
class InputExpensesWindow:
    def add_expense(self):
        # fill this in like the other one!
        pass

    def add_income(self):
        category = self.selected_income_category.get()
        amount = self.income_entry.get()
        category_id = None # you'll need to get the category_id from the database based on the category name
        description = None # add a description input field if you want this? not sure what this is for
        self.transaction_service.add_income(
        ...
        self.transaction_service.print_all_transactions(self.user_id)
    ...
    def __init__(self, user_id):
        ...
        self.past_entries = 4
        self.icons = {}
        for category in icon_dictionary:
            self.icons[category] = load_icon(category)
    ...
    def create_income_tab(self):
        ...
        with open(path, "r") as f:
            for line in f.readlines():
                ...
                entry = AmountEntry(self.icons[category], category, amount)
                ...
                entries += 1
                tree.insert("", tk.END, text=entry.category, image=entry.icon, values=(entry.amount))
...
class UserExpensesStats:
    ...
    def open_input_expenses_window(self):
        self.root.destroy()
        InputExpensesWindow(self.user.id)
...
class LoginWindow:
    def __init__(self, on_login):
        ...
        def login():
            ...
            user = self.on_login(email, password)
            if user:
                self.user = user
